...
/**
 *
 * `getMergePolicy()` provides by default the built-in model merge policy
 * to apply when extending a child model from a base model. This policy drives
 * the way parent/child model properties/settings are merged together.
 *
 * The following default merge policy applies to top-level settings properties:
 *
 * - Any
 ...
 *
 *
 * The built-in merge policy is as follows:
 *
 * ```
 * {
 ...
 * ```
 *
 * `getMergePolicy()` can be customized using model's setting `mergePolicy` as follows:
 *
 * ``` json
 * {
 * // ..
 ...
 *
 * ``` js
 * myModel.getMergePolicy = function() {
 *   const origin = myModel.base.getMergePolicy();
 ...
 */
ModelBaseClass.getMergePolicy = function() {
  const mergePolicy = {
    description: {replace: true}, // string or array
    options: {patch: true}, // object
    properties: {patch: true}, // object
  ...
  // current model might not have settings in case it is built-in ModelBaseClass
