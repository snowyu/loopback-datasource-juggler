 ...
 */
function mergeSettings(target, src, replaceTarget) { // NOTE: src is subclassSettings
  const mergePolicy = {
    description: {replace: true}, // string or array
    options: {innerReplace: true}, // object
    properties: {innerReplace: true}, // object
    hidden: {replace: false}, // array
    protected: {replace: false}, // array
    indexes: {innerReplace: true}, // object
    methods: {innerReplace: true}, // object
    mixins: {innerReplace: true}, // object
    // validations: {innerReplace: true}, // object // NOTE: not implemented
    relations: {innerReplace: true},
    scope: {replace: true}, // object
    scopes: {innerReplace: true}, // object
    acls: {replace: false}, // array
    // any matching non array/object property is replaced
    // any matching custom top-level object/array is replaced
  ...
  if (array) {
    ...
    let baseModelAclRank;
    if (target != null && typeof target === 'object') {
      // Add properties from target to dst
      Object.keys(target).forEach(function(key) {
        if (key === 'acls' && Array.isArray(target[key])) {
          let targetACLs = target[key];
          targetACLs.forEach(function(acl) {
            acl.rank = acl.rank || 1;
            // capture the base model ACL rank
            baseModelAclRank = baseModelAclRank || acl.rank;
          });
        }
        dst[key] = target[key];
      });
    ...
    if (src != null && typeof src === 'object') {
      ...
      Object.keys(src).forEach(function(key) {
        ...
        if (key === 'acls' && Array.isArray(src[key])) {
          let srcACLs = src[key];
          srcACLs.forEach(function(acl) {
            acl.rank = baseModelAclRank + 1;
          });
        }
        ...
        if (srcValue == null || typeof srcValue !== 'object') {
          ...
          if (target == null || typeof target !== 'object' || target[key] == null ||
           !mergePolicy[key] || mergePolicy[key].replace || replaceTarget) {
            // If target item is null, or not an object or should be replaced
            dst[key] = srcValue;
          } else {
            // mergeSettings() is called with replaceTarget true when the merge algorithm
            // should replace target's inner properties and stop merging recursively
            dst[key] = mergeSettings(target[key], src[key], mergePolicy[key].innerReplace);
          }
        }
      });
    }
