...
ModelBuilder.prototype.define = function defineClass(className, properties, settings, parent) {
  ...
  ModelClass.extend = function(className, subClassProperties, subClassSettings) {
    ...
    if (!subClassSettings.configurableModelMerge) {
      // legacy behaviour applying a deep merge of parent and child model settings
      subClassSettings = deepMerge(baseClassSettings, subClassSettings);
    } else {
      // configurable model merge behaviour if option is set
