...
ModelBuilder.prototype.define = function defineClass(className, properties, settings, parent) {
  ...
  ModelBaseClass.rank = ModelBaseClass.rank || 1;
  ...
  ModelClass.rank = ModelBaseClass.rank + 1;
  ...
  ModelClass.extend = function(className, subclassProperties, subclassSettings) {
    ...
    if (!true) { // TODO: define a feature flag value
      // legacy behaviour
      subclassSettings = mergeSettings(settings, subclassSettings);
    } else {
      // new behaviour
      let mergePolicy = ModelClass.mergePolicy;
      // deep clone settings
      ...
      Object.keys(settings).forEach(function(key) {
        ...
        if (mergePolicy[key] && mergePolicy[key].rank) {
          settings[key] = rankArrayElements(settings[key], ModelBaseClass.rank);
        }
      });

      Object.keys(subclassSettings).forEach(function(key) {
        // assign default merge policy to unknown settings properties
        mergePolicy[key] = mergePolicy[key] || mergePolicy.default;

        // allow null value to remove unwanted properties from base class settings
        if (subclassSettings[key] === null) {
          delete mergedSettings[key];
          return;
        }
        // rank sub class settings arrays elements where required
        if (mergePolicy[key].rank) {
          subclassSettings[key] = rankArrayElements(subclassSettings[key], ModelBaseClass.rank + 1);
        }
