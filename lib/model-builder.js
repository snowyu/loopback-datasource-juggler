...
ModelBuilder.prototype.define = function defineClass(className, properties, settings, parent) {
  ...
  // initializing base model inheritance rank if not set already
  ...
  // iterating sub model inheritance rank over base model rank
  ...
  // initialize getMergePolicy if not done already via model heritance
  ModelClass.getMergePolicy = ModelClass.getMergePolicy || function() {
    const mergePolicy = {
    ...
    // make sure we take settings of current model with `this`
    // model might have no settings in case it is an instance of builtin ModelBaseClass
    return (this.settings || {}).mergePolicy || mergePolicy;
  };

  // ModelClass.base might not have getMergePolicy() if it is an instance of builtin ModelBaseClass
  ModelClass.base.getMergePolicy = ModelClass.base.getMergePolicy || ModelClass.getMergePolicy;
  ...
  ModelClass.extend = function(className, subclassProperties, subclassSettings) {
    ...
    if (!subclassSettings.enableOptionatedModelMerge) {
      // legacy behaviour
      subclassSettings = mergeSettings(settings, subclassSettings);
    } else {
      // optionated model merge behaviour if option is set

      // get the mergePolicy specification
      let mergePolicy = ModelClass.getMergePolicy();

      // deep clone base class settings
      let mergedSettings = mergeSettings(settings);

      Object.keys(settings).forEach(function(key) {
        // rank base class settings arrays elements where required
        if (mergePolicy[key] && mergePolicy[key].rank) {
          settings[key] = rankArrayElements(settings[key], ModelBaseClass.__rank);
        }
      });

      Object.keys(subclassSettings).forEach(function(key) {
        // assign default merge policy to unknown settings
        mergePolicy[key] = mergePolicy[key] || mergePolicy.__default;

        // allow null value to remove unwanted properties from base class settings
        ...
        mergedSettings[key] = mergeSettings(settings[key], subclassSettings[key]);
      });
      // assign final sub class settings
      subclassSettings = mergedSettings;
    }
